...
MPERS_PRINTER_DECL(void, tprint_msgbuf, struct tcb *const tcp, const kernel_ulong_t addr, const kernel_ulong_t count)
{
 ...
 if (!umoven_or_printaddr((tcp), (addr), sizeof(*(&msg)), (void *) (&msg))) {
  tprint_struct_begin();
  do { tprints_field_name("mtype"); tprintf_string("%lld", (sizeof((msg).mtype) == sizeof(char) ? (long long) (signed char) ((msg).mtype) : sizeof((msg).mtype) == sizeof(short) ? (long long) (short) ((msg).mtype) : sizeof((msg).mtype) == sizeof(int) ? (long long) (int) ((msg).mtype) : sizeof((msg).mtype) == sizeof(long) ? (long long) (long) ((msg).mtype) : (long long) ((msg).mtype))); } while (0);
  tprint_struct_next();
