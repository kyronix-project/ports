...
const struct xlat perf_ioctl_flags[1] = { {
 ...
 .size = (sizeof(perf_ioctl_flags_xdata) / sizeof((perf_ioctl_flags_xdata)[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(__typeof__((perf_ioctl_flags_xdata)), __typeof__(&(perf_ioctl_flags_xdata)[0])))]) * 0)),
 .type = XT_NORMAL,
 .flags_mask = 0
  | ((unsigned) ((1U << 0)))
  ,
...
perf_ioctl_query_bpf(struct tcb *const tcp, const kernel_ulong_t arg)
{
 ...
 if ((!((tcp)->flags & 0x04))) {
  ...
  tprintf_string("%llu", (sizeof(info) == sizeof(char) ? (unsigned long long) (unsigned char) (info) : sizeof(info) == sizeof(short) ? (unsigned long long) (unsigned short) (info) : sizeof(info) == sizeof(int) ? (unsigned long long) (unsigned int) (info) : sizeof(info) == sizeof(long) ? (unsigned long long) (unsigned long) (info) : (unsigned long long) (info)));
  tprint_struct_next();
  return 0;
 }
 if (((tcp)->u_error != 0) ||
 ...
 print_array(tcp, arg + __builtin_offsetof(struct perf_event_query_bpf, ids), info,
      &info, sizeof(info),
      tfetch_mem, print_uint_array_member, ((void*)0));
 tprint_struct_end();
 return 0200;
}
static int
