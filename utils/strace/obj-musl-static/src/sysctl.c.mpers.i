...
int sys_sysctl(struct tcb *tcp)
{
 ...
 if (umoven_or_printaddr((tcp), (tcp->u_arg[0]), sizeof(*(&info)), (void *) (&info)))
  return 0100;
 tprint_struct_begin();
 do { tprints_field_name("name"); printaddr((mpers_ptr_t) (info).name); } while (0);
 tprint_struct_next();
 do { tprints_field_name("nlen"); tprintf_string("%lld", (sizeof((info).nlen) == sizeof(char) ? (long long) (signed char) ((info).nlen) : sizeof((info).nlen) == sizeof(short) ? (long long) (short) ((info).nlen) : sizeof((info).nlen) == sizeof(int) ? (long long) (int) ((info).nlen) : sizeof((info).nlen) == sizeof(long) ? (long long) (long) ((info).nlen) : (long long) ((info).nlen))); } while (0);
 tprint_struct_next();
 do { tprints_field_name("oldval"); printaddr((mpers_ptr_t) (info).oldval); } while (0);
 tprint_struct_next();
 do { tprints_field_name("oldlenp"); printaddr((mpers_ptr_t) (info).oldlenp); } while (0);
 tprint_struct_next();
 do { tprints_field_name("newval"); printaddr((mpers_ptr_t) (info).newval); } while (0);
 tprint_struct_next();
 do { tprints_field_name("newlen"); tprintf_string("%llu", (sizeof((info).newlen) == sizeof(char) ? (unsigned long long) (unsigned char) ((info).newlen) : sizeof((info).newlen) == sizeof(short) ? (unsigned long long) (unsigned short) ((info).newlen) : sizeof((info).newlen) == sizeof(int) ? (unsigned long long) (unsigned int) ((info).newlen) : sizeof((info).newlen) == sizeof(long) ? (unsigned long long) (unsigned long) ((info).newlen) : (unsigned long long) ((info).newlen))); } while (0);
 tprint_struct_end();
 return 0100;
}
