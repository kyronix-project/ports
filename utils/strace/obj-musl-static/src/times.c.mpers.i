...
int sys_times(struct tcb *tcp)
{
 ...
 if (((tcp)->flags & 0x04) && !umoven_or_printaddr((tcp), (tcp->u_arg[0]), sizeof(*(&tbuf)), (void *) (&tbuf))) {
  tprint_struct_begin();
  do { tprints_field_name("tms_utime"); print_clock_t((tbuf).tms_utime); } while (0);
  tprint_struct_next();
  do { tprints_field_name("tms_stime"); print_clock_t((tbuf).tms_stime); } while (0);
  tprint_struct_next();
  do { tprints_field_name("tms_cutime"); print_clock_t((tbuf).tms_cutime); } while (0);
  tprint_struct_next();
  do { tprints_field_name("tms_cstime"); print_clock_t((tbuf).tms_cstime); } while (0);
  tprint_struct_end();
 }
 return 0;
}
