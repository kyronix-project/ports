...
;
_Static_assert((1) == (1), "SS_ONSTACK != 1");
...
#pragma GCC diagnostic ignored "-Wtautological-compare"
;
_Static_assert((2) == (2), "SS_DISABLE != 2");
...
#pragma GCC diagnostic ignored "-Wtautological-compare"
;
_Static_assert(((1U << 31)) == ((1U << 31)), "SS_AUTODISARM != (1U << 31)");

#pragma GCC diagnostic pop
;

static const struct xlat_data sigaltstack_flags_xdata[] = {
 { (unsigned)(1), "SS_ONSTACK" },
 { (unsigned)(2), "SS_DISABLE" },
 { (unsigned)((1U << 31)), "SS_AUTODISARM" },
};
static
const struct xlat sigaltstack_flags[1] = { {
 .data = sigaltstack_flags_xdata,
 .size = (sizeof(sigaltstack_flags_xdata) / sizeof((sigaltstack_flags_xdata)[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(__typeof__((sigaltstack_flags_xdata)), __typeof__(&(sigaltstack_flags_xdata)[0])))]) * 0)),
 .type = XT_NORMAL,
 .flags_mask = 0
  | ((unsigned) (1))
  | ((unsigned) (2))
...
print_stack_t(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 tprint_struct_next();
 do { tprints_field_name("ss_size"); tprintf_string("%llu", (sizeof((ss).ss_size) == sizeof(char) ? (unsigned long long) (unsigned char) ((ss).ss_size) : sizeof((ss).ss_size) == sizeof(short) ? (unsigned long long) (unsigned short) ((ss).ss_size) : sizeof((ss).ss_size) == sizeof(int) ? (unsigned long long) (unsigned int) ((ss).ss_size) : sizeof((ss).ss_size) == sizeof(long) ? (unsigned long long) (unsigned long) ((ss).ss_size) : (unsigned long long) ((ss).ss_size))); } while (0);
 tprint_struct_end();
}
