...
MPERS_PRINTER_DECL(void, print_kernel_sigset, struct tcb *tcp, const kernel_ulong_t addr)
{
 ...
 if (!umoven_or_printaddr((tcp), (addr), sizeof(*(&sas)), (void *) (&sas))) {
  ...
  print_sigset_addr_len(tcp, (uintptr_t) sas.sigmask,
          sas.sigsetsize);
  tprint_struct_next();
  do { tprints_field_name("sigsetsize"); tprintf_string("%llu", (sizeof((sas).sigsetsize) == sizeof(char) ? (unsigned long long) (unsigned char) ((sas).sigsetsize) : sizeof((sas).sigsetsize) == sizeof(short) ? (unsigned long long) (unsigned short) ((sas).sigsetsize) : sizeof((sas).sigsetsize) == sizeof(int) ? (unsigned long long) (unsigned int) ((sas).sigsetsize) : sizeof((sas).sigsetsize) == sizeof(long) ? (unsigned long long) (unsigned long) ((sas).sigsetsize) : (unsigned long long) ((sas).sigsetsize))); } while (0);
  tprint_struct_end();
 }
}
